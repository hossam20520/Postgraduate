...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.register.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.registers.update", [$register->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="studentname">{{ trans('cruds.register.fields.studentname') }}</label>
                <input class="form-control {{ $errors->has('studentname') ? 'is-invalid' : '' }}" type="text" name="studentname" id="studentname" value="{{ old('studentname', $register->studentname) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('studentid') ? 'is-invalid' : '' }}" type="text" name="studentid" id="studentid" value="{{ old('studentid', $register->studentid) }}">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('email') ? 'is-invalid' : '' }}" type="email" name="email" id="email" value="{{ old('email', $register->email) }}" required>
                @if($errors->has('email'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('phone') ? 'is-invalid' : '' }}" type="text" name="phone" id="phone" value="{{ old('phone', $register->phone) }}" required>
                @if($errors->has('phone'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('subjects') ? 'is-invalid' : '' }}" name="subjects[]" id="subjects" multiple>
                    @foreach($subjects as $id => $subject)
                        <option value="{{ $id }}" {{ (in_array($id, old('subjects', [])) || $register->subjects->contains($id)) ? 'selected' : '' }}>{{ $subject }}</option>
                    @endforeach
                </select>
